...
def download_and_extract_dataset(name: str, url: str, output: Path) -> None:
    ...
    if not zip_file.exists():
        ...
        if "drive.google.com" in url:
            gdown.download(url, str(zip_file))
        else:
            torch.hub.download_url_to_file(url, str(zip_file))
...
def main():
    ...
    out_dir = Path("data")
    download_and_extract_dataset(args.name, URLS[args.name], out_dir)
